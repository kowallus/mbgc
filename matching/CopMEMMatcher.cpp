...
void CopMEMMatcher::displayParams() {
	...
	*PgHelpers::logout << "Hash function: " << (H == 2?"xxHash32":"please check H value") << std::endl;
    *PgHelpers::logout << "Hash collisions per position limit: "
...
HashBuffer<MyUINT1, MyUINT2> CopMEMMatcher::processRef() {
    if (!collisionSupport)
        return processIgnoreCollisionsRef<MyUINT1, MyUINT2>();
...
template<typename MyUINT1, typename MyUINT2>
HashBuffer<MyUINT1, MyUINT2> CopMEMMatcher::processIgnoreCollisionsRef() {
    const unsigned int MULTI2 = 128;
    const unsigned int k1MULTI2 = k1 * MULTI2;

    MyUINT1* sampledPositions = new MyUINT1[hash_size];
...
template<typename MyUINT1, typename MyUINT2>
void CopMEMMatcher::processExactMatchQueryIgnoreCollisionsTight(HashBuffer<MyUINT1, MyUINT2> buffer, vector<TextMatch> &resMatches,
    ...
    for (; i1 + K < N2 + 1; i1 += k2) {
        ...
        if (r1 == r2 || l1 == l2) {
            ...
            for (int i = 0; i < blocks; i += 2, p1 += 2 * MEMCMP_BLOCK, p2 += 2 * MEMCMP_BLOCK) {
                if (memcmp(p1, p2, MEMCMP_BLOCK) != 0)
                    break;
                if (memcmp(p1 + MEMCMP_BLOCK, p2 + MEMCMP_BLOCK, MEMCMP_BLOCK) != 0) {
            ...
            p1 = curr1;
            p2 = curr2;
            while (p1 != start1 - 1 && p2 != start2 - 1 && *--p1 == *--p2);

            if (right - p1 > minMatchLength && memcmp(curr1, curr2, K) == 0) {
                resMatches.push_back(TextMatch(p1 + 1 - start1, right - p1 - 1, (p2 + 1 - start2)));
                const int skip = ((p2 - start2 + right - p1) - (curr2 - start2)) / k2;
                const int skipK2 = skip * k2;
                curr2 += skipK2;
                i1 += skipK2;
                continue;
...
uint64_t CopMEMMatcher::processApproxMatchQueryTight(HashBuffer<MyUINT1, MyUINT2> buffer, const char *start2,
                                                     const uint16_t N2, uint8_t maxMismatches,
    ...
    const uint16_t N2trim8 = (N2 / sizeof(uint64_t)) * sizeof(uint64_t);
    uint16_t k2positionsCount = (N2 + 1 - K) / k2;
    ...
    for (; i1 + K < N2 + 1; i1 += k2) {
        ...
        for (MyUINT1 j = posArray[0]; j < posArray[1]; ++j) {
            const uint16_t positionShift = curr2 - start2;
...
CopMEMMatcher::CopMEMMatcher(const char *srcText, const size_t srcLength, const uint32_t targetMatchLength,
                             bool collisionSupport, uint32_t minMatchLength)
    : start1(srcText), N(srcLength), L(targetMatchLength), collisionSupport(collisionSupport) {
    initHashFuncMatrix();
...
void
CopMEMMatcher::matchTexts(vector<TextMatch> &resMatches, const char* destText, size_t destLen, bool destIsSrc,
...
uint64_t CopMEMMatcher::approxMatchPattern(const char *pattern, const uint16_t length, uint8_t maxMismatches, uint8_t minMismatches,
