...
PgHelpers::VarLenDNACoder::VarLenDNACoder(const string &codes) {
    initUsing(codes);
}

void PgHelpers::VarLenDNACoder::initUsing(const string &codes) {
    int sPos = 0;
    int pos = 0;
    int counter = 0;
    ...
    for (int i = 0; i < counter; i++) {
        ...
        codeLUT[getLUTidx(temp)] = i;
    }
}

PgHelpers::VarLenDNACoder::~VarLenDNACoder() {
    delete[] codeLUT;
}

void
PgHelpers::VarLenDNACoder::writeBook(unsigned char *dest, size_t &destLen) {
...
void
PgHelpers::VarLenDNACoder::encode(unsigned char *dest, size_t &destLen, const unsigned char *src, size_t srcLen) {
    ...
    while (pos <= srcLen - 4) {
        ...
        temp &= CODE_LUT_3_MASK;
        if (codeLUT[temp] != NOT_FOUND_CODE) {
            dest[destLen++] = codeLUT[temp];
            pos += 3;
            continue;
        }
        temp &= CODE_LUT_2_MASK;
        if (codeLUT[temp] != NOT_FOUND_CODE) {
            dest[destLen++] = codeLUT[temp];
            pos += 2;
            continue;
        }
        temp &= CODE_LUT_1_MASK;
        if (codeLUT[temp] == NOT_FOUND_CODE && temp != (65 & CODE_LUT_B1_MASK)) {
            printf("ERROR: var-len encoder unexpected encoutered symbol!\n");
            exit(EXIT_FAILURE);
        }
        dest[destLen++] = codeLUT[temp];
        pos++;
...
int PgHelpers::VarLenDNACoder::decode(unsigned char *dest, size_t expDestLen, unsigned char *src, size_t srcLen) {
    uint8_t* destPtr = dest;
...
int
PgHelpers::VarLenDNACoder::Compress(unsigned char *&dest, size_t &destLen, const unsigned char *src, size_t srcLen,
    ...
    dest[0] = props->mode;
    dest[1] = props->staticCodeBookID;
    switch (props->mode) {
        case STATIC_CODES_CODER_PARAM:
            coder = new VarLenDNACoder(props->staticCodeBookID);
            break;
        default:
            fprintf(stderr, "Unsupported %d PgRC var-len coder parameter.\n", (int) props->staticCodeBookID);
            exit(EXIT_FAILURE);
    }
...
string PgHelpers::VarLenDNACoder::readBook(unsigned char *src) {
    return string((char*) src);
}

int PgHelpers::VarLenDNACoder::Uncompress(unsigned char *dest, size_t *destLen, unsigned char *src, size_t *srcLen) {
...
PgHelpers::VarLenDNACoder::VarLenDNACoder(uint8_t staticCodeBookID) {
...
const string PgHelpers::VarLenDNACoder::AG_EXTENDED_CODES =
        "A\nAAA\nAAAA\nGAAA\nCAA\nACAA\nGCAA\nGAA\nAGAA\nGGAA\nTAA\nATAA\nGTAA\nACA\nAACA\nGACA\nCCA\nACCA\nGCCA\nGCA\n"
        "AGCA\nGGCA\nTCA\nATCA\nGTCA\nAGA\nAAGA\nGAGA\nCGA\nACGA\nGCGA\nGGA\nAGGA\nGGGA\nTGA\nATGA\nGTGA\nATA\nAATA\n"
        "GATA\nCTA\nACTA\nGCTA\nGTA\nAGTA\nGGTA\nTTA\nATTA\nGTTA\nTG%\nT%\nAT%\nCT%\nGT%\nTT%\n"
        "N\nAN\nAAN\nCAN\nGAN\nNAN\nTAN\n"
...
const string PgHelpers::VarLenDNACoder::SYNC_ON_A_CODES =
        "A\nAA\nAAA\nAAAA\nACAA\nAGAA\nATAA\nACA\nAACA\nACCA\nAGCA\nATCA\nAGA\nAAGA\nACGA\nAGGA\nATGA\nATA\nAATA\nACTA\n"
        "AGTA\nATTA\n"
        "\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n"
        "C\nAC\nAAC\nAAAC\nACAC\nAGAC\nATAC\nCC\nACC\nAACC\nCCC\nACCC\nGCC\nAGCC\nTCC\nATCC\nGC\nAGC\nAAGC\nCGC\nACGC\n"
...
const string PgHelpers::VarLenDNACoder::AG_SHORT_EXTENDED_CODES =
        "A\nAA\n\n\nAAA\n\n\n\n\n\nGAA\n\n\nCA\n\n\nACA\n\n\n\n\n\nGCA\n\n\nGA\n\n\nAGA\n\n\n\n\n\nGGA\n\n\nTA\n\n\n"
        "ATA\n\n\n\n\n\nGTA\n\n\nTG%\nT%\nAT%\nCT%\nGT%\nTT%\n"
        "N\nAN\nAAN\nCAN\nGAN\nNAN\nTAN\n"
        "\n\n"
